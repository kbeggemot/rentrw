...
export default function PartnersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return partners.filter((p) =>
      p.phone.toLowerCase().includes(q) || (p.fio ? p.fio.toLowerCase().includes(q) : false)
    );
  }, [partners, query]);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">Партнеры</h1>
      ...
      <div className="overflow-x-auto bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-lg">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-3 py-6 text-center text-gray-500">Нет партнеров</td>
              </tr>
            ) : filtered.map((p) => (
