...
async function findSaleMappingByTask(taskId: string): Promise<Mapping | null> {
  // 1) Fast path: by-task index
  try {
    const raw = await readText(`.data/sales_index/by_task/${encodeURIComponent(String(taskId))}.json`);
    if (raw) {
      const d = safeJsonParse(raw);
      const inn = typeof d?.inn === 'string' ? d.inn : null;
      const userId = typeof d?.userId === 'string' ? d.userId : null;
      if (inn && userId) return { inn, userId, taskId };
    }
  } catch {}
  ...
  for (const p of idxKeys) {
    ...
    try {
      ...
      for (const r of rows) {
        if (String((r as any)?.taskId) === String(taskId)) {
...
export async function GET(req: Request) {
  try {
    ...
    const taskId = decodeURIComponent(segs[segs.length - 1] || '').trim();
    if (!taskId) return NextResponse.json({ ok: false, error: 'BAD_TASK' }, { status: 400 });

    let mapping = await findSaleMappingByTask(taskId);
    let sale: any | null = null;

    if (mapping) sale = await readSaleByMapping(mapping);

    // Fallback: legacy monolithic store
    if (!sale) {
      try {
        const all = await listAllSales();
        const found = all.find((s: any) => String(s?.taskId) === String(taskId)) || null;
        if (found) {
          mapping = mapping || { inn: sanitizeInn(found?.orgInn || null), userId: String(found?.userId || ''), taskId };
          sale = found;
        }
      } catch {}
    }
    ...
    const orderId = normalizeOrderId(sale?.orderId);
    ...
    const whyNoFull = (() => {
      ...
      if (expectation.path === 'full_same_day') {
        ...
        return 'ofdFullId есть, но ссылка (ofdFullUrl) ещё не подтянулась (колбэк OFD или фоновые воркеры)';
      }
