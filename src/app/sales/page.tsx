...
export default function SalesPage() {
  ...
  const load = async (refresh = false) => {
    setLoading(true);
    try {
      // 1) сначала показываем текущие данные без ожидания обновления
      if (refresh) {
        // параллельно запускаем обновление, но без блокировки UI
        ...
        // мгновенно грузим старые данные для отображения
  ...
  const filtered = useMemo(() => {
    ...
    const toTs = dateTo ? new Date(dateTo).getTime() + 24 * 60 * 60 * 1000 - 1 : null; // inclusive day end
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50 dark:bg-gray-900">
            <tr>
              ...
              <th className="text-left px-3 py-2">Чек на покупку</th>
